...
### 32.6 — Billing: Square Subscriptions is explicitly stubbed, not faked

**No real money can move through this feature yet.** The client-side gate (`SQUARE_SUBSCRIPTION_PLAN_ID` in `public/index.html`, empty by default), the database schema, and a stub serverless function (`netlify/functions/create-subscription-checkout.js`, modeled on the existing `create-checkout.js` pattern) are all in place and tested, but the real Square Subscriptions API call — creating a Catalog subscription-plan object in the Square dashboard, generating a Subscriptions-API-scoped access token distinct from the payment-link tokens the one-time tiers already use, and building a webhook receiver to keep `subscribers.status` accurate over time — is deliberately not implemented. While `SQUARE_SUBSCRIPTION_PLAN_ID` is empty, the Subscribe button shows an honest "Subscriptions Launching Soon" state and never attempts a checkout that could fail or mis-charge someone. See `docs/SUBSCRIPTIONS_SETUP.md`'s "SQUARE SUBSCRIPTIONS — NOT YET LIVE" section for the full, step-by-step remainder of this work. No Square plan ID, access token, or webhook secret is fabricated or hardcoded anywhere in this codebase or its documentation.

### 32.7 — What this explicitly does not do

This Part does not touch partnership or group reading code in any way — like the Nodal Choice interface it builds on (Part XXXI §31.6), it is solo-reading-only. It does not alter, duplicate, or reinterpret `doReading()`, `getFallbackReading()`, `buildNodalChoiceSection()`, `getNodeDecanCards()`, or any other existing reading function — every one of them is called, never rewritten. It does not change how `TIERS`, `GROUP_TIERS`, or `ARCHETYPE_QUIZ_TIER` purchases work, and never requires sign-in for any of them. It does not claim the Lookback's synthesis mechanic is inherited tradition — see §32.2. And it does not charge, or claim to be able to charge, a single real dollar until the steps named in §32.6 are actually completed by a human, outside of this codebase.
...
3. **Azure AI Speech upgrade** (pre-existing, untouched) — the paid tier, using Azure's named neural voices ("Aria," "Guy," "Natasha," etc.) via an admin-supplied API key and region.

Tier 2 sits genuinely *between* the other two: strictly better-sounding than tier 1's robotic default, strictly more limited than tier 3's paid, professionally-produced named voices — and, unlike tier 3, it costs nothing and needs no account or key.

### 33.3 — Honest scope and limitations, stated plainly

- **A fixed, small voice set — not arbitrary named voices.** Kokoro ships exactly 28 built-in English voices (11 American female, 9 American male, 4 British female, 4 British male), listed by their real names in the admin panel's picker. There is no "Natasha," no custom voice cloning, and no way to add more without a model update from its own maintainers. This is explicitly different from Azure's named-voice roster (tier 3) and is said so directly in the admin panel copy, not left for someone to discover only after enabling it.
