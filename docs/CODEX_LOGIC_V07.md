...
## Part LI — Android's Free Voice: "Google", and Why It's Likely Already Covered

### 51.1 — The question and what was checked before answering

The project owner asked directly whether a free Google/Android-compatible voice exists, parallel to Part L's "Natasha" for Windows. Rather than assume the same name-search trick would work identically, this was checked via live web search first: Android's own built-in TTS engine ("Speech Services by Google") IS free and pre-installed on virtually every Android device — but Chrome on Android's `getVoices()` list behaves differently from Windows/iOS. It is real, but notably messier: it does not reliably expose clean, human-readable names the way desktop Chrome or iOS Safari do, and offers no guaranteed way to pick an exact named voice by string search on every Android/Chrome combination.

The good news, reasoned through rather than assumed: when neither a specifically-named voice matches (the existing `_resolvedBrowserVoice` fallback), `utter.voice` stays unset, and the browser plays its own current default voice — which, on the overwhelming majority of real Android phones, already IS this same free Google voice, since "Speech Services by Google" ships as the OS's default TTS engine. So Android was very likely already receiving the free Google voice automatically, even before this Part, purely through the existing unset-voice fallback behavior.

### 51.2 — The fix: a defensive `pickGoogleVoice()`, same pattern as Natasha

Added anyway, as a best-effort, zero-risk safety net for the browser/OS combinations where a named "Google ..." voice IS exposed: `pickGoogleVoice(voices)`, matching any voice name containing "Google" (case-insensitive, word-boundary), tried in the existing chain between Natasha and Aria: `pickNatashaVoice(voices) || pickGoogleVoice(voices) || pickAriaVoice(voices)`. Each of the three only ever matches on the platform it is actually native to (Natasha: Windows; Google: Android, when named; Aria: Windows/Edge), so there is no real risk of one platform's preference accidentally winning on another — exactly the same graceful, per-platform fallback design Part L already established, just extended by one more name.

### 51.3 — Verification (this session)

All 5 inline `<script>` blocks pass `node --check`; diff confined to the new `pickGoogleVoice()` function and its one call site's chain.

Directly tested the full three-way resolution chain (`pickNatashaVoice() || pickGoogleVoice() || pickAriaVoice()`) against four real voice-list shapes: (1) a Windows list with both Natasha and Aria present — Natasha wins, unaffected by the new Google check; (2) an Android list with named Google voices present ("Google US English", "Google UK English Female") — the new Google match correctly wins; (3) a realistic messy/unnamed Android voice list (raw internal identifiers like `en-us-x-iol-local`, matching the real-world behavior confirmed in §51.1) — correctly returns `null`, falling through to the browser's own default rather than erroring or hanging; (4) an iPhone's real voice names (Samantha, Karen, Alex) — correctly returns `null`, same as before this Part.

**What could not be verified in this sandbox:** whether a real Android/Chrome combination's voice list actually contains the word "Google" in practice, and — the more important claim — whether Android's own default voice (the one used when this whole chain falls through to `null`) genuinely is the free Google voice on a real device, as reasoned in §51.1. No real Android device or browser exists in this sandbox to confirm either directly.
